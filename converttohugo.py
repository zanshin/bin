...
#    print "Post date: ", post_date.date()
    Q = "'"
    front_data['date'] = Q + post_date.strftime('%m/%d/%Y') + Q
    # front_data['date'] = post_date.strftime('%m/%d/%Y')
    ...
    for root, dirs, files in os.walk(src_dir):
        for filename in files:
            try:
                ...
                if os.path.splitext(filename)[1] != '.markdown' or filename in ['README.md', 'LICENSE.md']:
                    continue
